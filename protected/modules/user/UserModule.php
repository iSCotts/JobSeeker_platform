...
class UserModule extends CWebModule
{
	...
	public $returnLogoutUrl = array("/user/login");
        public $skillUrl = array("/user/skill");
        public $mySkillsUrl = array("/user/userSkill/ViewMySkills");
        public $viewContractorsUrl = array("/user/user/viewContractors");
        public $notificationUrl = array("/user/Notifications");
        public $viewAccount = array("/user/account/viewAccount");
        public $makeDeposit = array("/user/account/Deposit");
        public $managePayments = array("/user/account/ManagePayments");
	...
	public $tableUsers = 'tbl_user';
	public $tableProfiles = 'tbl_profile';
	...
	public static function user($id=0) {
        ...
        public function getUserName()
        {
            ...
            if($user!=null)
            {
                return $name = $user->profile->firstname;// . " " . $user->profile->lastname;
            }
            else
            {
	...
	public function users() {
        ...
        public function  notifyUserAboutJob($jid,$uid)
        {
            $noteType= NotificationType::model()->findByAttributes(array('type'=>'awarded_job'));
            $note = new Notifications;
            $note->receiver_id =$uid;
            $note->job_post_id=$jid;
            $note->notification_type_id=$noteType->id;
            $note->save();
        }
        
        public function notifyUserAboutInvite($jid,$uid)
        {
            $noteType= NotificationType::model()->findByAttributes(array('type'=>'job_invite'));
            if($noteType!=null)
            {
                $note = new Notifications;
                $note->receiver_id =$uid;
        ...
        public function notifyUserAboutPayment($jid,$uid)
        {
            $noteType= NotificationType::model()->findByAttributes(array('type'=>'pay_received'));
            if($noteType!=null)
            {
                $note = new Notifications;
                $note->receiver_id =$uid;
        ...
        public static function  inviteUsers($users=array(),$jid)
        {
            foreach ($users as $user)
            {
                ...
                $invite->user_id=$user;
                $invite->job_post_id = $jid;
                if($invite->save())
                {
                    yii::app()->getModule('user')->notifyUserAboutInvite($jid, $user);
                }
        ...
        public function getNotificationCount()
        {
            ...
            $data= new CActiveDataProvider('Notifications',array(
                        'criteria'=>$criteria,
                    ));
